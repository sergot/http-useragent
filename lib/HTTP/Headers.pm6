...
our grammar HTTP::Headers::Grammar {
    token TOP {
        [ <message-header> "\r\n" ]*
    }
...
our class HTTP::Headers::Actions {
...
method new(*%headers) {
    ...
    for %headers.kv -> $k, $v {
        @headers.push: HTTP::Header.new(:name($k), :values($v.list));
    }
...
multi method header(*%headers) {
    for %headers.kv -> $k, $v {
        my $h = HTTP::Header.new(:name($k), :values($v.list));
...
method parse($raw) {
    my $*OBJ = self;
    HTTP::Headers::Grammar.parse($raw, :actions(HTTP::Headers::Actions));
}
